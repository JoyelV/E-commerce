...
const updatepaymentStatus = async (req, res) => {
    try {
        const { razorpay_payment_id, razorpay_order_id, razorpay_signature, orderID, data, paymentStatus } = req.body;
        ...
        if (paymentStatus === 'success') {
            ...
            for (const item of cart.items) {
                ...
                finalSpecialDiscount += specialDiscount;
            }
        ...
        if (paymentOption === 'COD') {
            ...
            for (const item of cart.items) {
                ...
                if (products.countInStock < item.quantity) {
                    return res.status(400).json({ message: `Product ${products.name} is out of stock` });
                }
            ...
            const billTotal = sum + cart.deliveryCharges;
            var order_id = await generateUniqueOrderID();

            const options = {
                amount: Math.round(billTotal * 100), // amount in the smallest currency unit
                currency: "INR",
                receipt: order_id
            };
